...
void InitSystemTiming()
{
    ...
    Timer32_setCount(TIMER32_1_BASE, LOADVALUE);
    ...
    //Clearing interrupt flag and enabling it for timerA
    Timer_A_clearInterruptFlag(TIMER_A2_BASE);
    ...
    Timer32_initModule(TIMER32_1_BASE, // There are two timers, we are using the one with the index 0
     ...
     Timer32_registerInterrupt(INT_T32_INT2, TimerExpired);

     Timer32_clearInterruptFlag(TIMER32_1_BASE);

     Interrupt_enableInterrupt(INT_T32_INT2);

     Timer_A_registerInterrupt(TIMER_A2_BASE, TIMER_A_CCR0_INTERRUPT, TimerExpired);
